...
with ui.layout_columns():
    with ui.card(full_screen=True):
        ...
        def confusion_matrix_shiny():
            cm = confusion_matrix(
                filtered_df()["results"], filtered_df()["prediction_value"]
            )
    ...
    with ui.card(full_screen=True):
        ...
        def summary_statistics():
            render_df = filtered_df()[
                ["id_season", "game_date", "tm", "opp", "results", "prediction_value"]
            ].copy()
            render_df["game_date"] = render_df["game_date"].astype(str).str.slice(0, 10)
            return render.DataGrid(
                render_df.sort_values(by="game_date", ascending=False), filters=True
            )
